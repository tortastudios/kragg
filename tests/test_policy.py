...
def test_crag_toml_takes_precedence_over_pyproject(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text(
        '[tool.crag]\nprofile = "from-pyproject"\n'
    )
    (tmp_path / "crag.toml").write_text(
        'profile = "from-crag-toml"\nmax_violations_per_gate = 5\n'
    )

    policy = load_policy(tmp_path)

    assert policy.profile == "from-crag-toml"
